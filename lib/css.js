...
module.exports = function (app) {
        ...
        function (next) {
          try {
            ...
            app.cache.get(cacheKey, function (err, result) {
              ...
              // If no result, continue to build assets.
            ...
            // This step is optional, continue if `cantina-cache` was not found.
            if (e.code === 'MODULE_NOT_FOUND') return next();
            next(e);
          }
        },
  ...
  api.cache = function cache (namespace, cb) {
    try {
      ...
      // Cache the content.
      if (app.assets.css[namespace].content) {
        app.log('- caching `' + namespace + '` css ...');

        var cacheKey = 'assets:' + app.assets.css[namespace].hash;
        app.cache.set(cacheKey, app.assets.css[namespace].content, cb);
      }
      else {
        cb();
      }
      ...
      // This step is optional, continue if `cantina-cache` was not found.
      if (e.code === 'MODULE_NOT_FOUND') return cb();
      cb(e);
    }
  };
