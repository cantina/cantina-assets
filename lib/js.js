    ...
    async.series([
      ...
      function (next) {
        ...
        dirs.forEach(function (dir) {
          dir = (conf.root || app.root) + dir + '/public/';
          glob.sync(dir + '**/*.js').forEach(function (file) {
            var key = file.substr(dir.length);
      ...
      function (next) {
        ...
        var confFile;
        dirs.forEach(function (dir) {
          var file = (conf.root || app.root) + dir + '/public/js/config.js';
      ...
      function (next) {
        ...
        async.reduce(
          ...
          function (err, result) {
            ...
            app.assets.js[namespace].basePath = '/assets/js/' + app.assets.js[namespace].hash + '-' + namespace;
            next();
          }
        );
      },
...
exports.minify = function minify (namespace, cb) {
  if (app.conf.get('assets:js:' + namespace + ':minify') && app.assets.js[namespace].content) {
    ...
    var source = new File({
      cwd: '/',
      base: '',
      path: app.assets.js[namespace].basePath + '.js',
      contents: new Buffer(app.assets.js[namespace].content)
    });

    // Minify
    uglyfly.minify(source, {
      //fromString: true,
      outSourceMap: true,
      //sourceMapIncludeSources: true,
      sourceMapURL: app.assets.js[namespace].basePath + '.js.map',
      mangle: {
        except: ['require', 'requirejs', 'define']
      }
    }, function (err, data) {
      if (err) return cb(err);
      app.assets.js[namespace].min = data.code;
      if (data.map) {
        app.assets.js[namespace].map = data.map;
      }
      cb();
    });
  }
...
exports.serve = function serve (namespace, cb) {
  if (app.conf.get('assets:js:' + namespace + ':serve') && app.assets.js[namespace].content) {
    ...
    app.middleware.get(-1200, app.assets.js[namespace].basePath + '.js', dish(app.assets.js[namespace].content, {
      headers: {
        'Content-Type': 'text/javascript'
      }
    }));
    if (app.assets.js[namespace].min) {
      app.middleware.get(-1200, app.assets.js[namespace].basePath + '.min.js', dish(app.assets.js[namespace].min, {
        headers: {
          'Content-Type': 'text/javascript'
        }
      }));
      // Override conf js with the aggregate.
      reset.push(app.assets.js[namespace].basePath + '.min.js');
      app.conf.set('scripts', reset);
    }
    else {
      // Override conf js with the aggregate.
      reset.push(app.assets.js[namespace].basePath + '.js');
      app.conf.set('scripts', reset);
    }
    if (app.assets.js[namespace].map) {
      app.middleware.get(-1200, app.assets.js[namespace].basePath + '.js.map', dish(app.assets.js[namespace].map, {
        headers: {
