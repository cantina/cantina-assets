...
module.exports = function (app) {
  ...
  api.optimize = function optimize (cb) {
    ...
    settings = app.conf.get('assets:templates') || {};
    namespaces = Object.keys(settings);
          ...
          if (matches) {
            ...
            matches.forEach(function (name) {
              ...
              if (name.match(/^(?:[.\/])*shared/)) {
                lookup = files[name.replace('shared/', 'shared/templates/')];
              }
              else {
                lookup = files['templates/' + name];
              }

              // Warn about skipped files.
              if (!lookup) {
                console.warn('  - Missing (or dynamic): ' + name);
              }
              else {
                templates[name] = lookup;
              }
            });
          }
          next();
